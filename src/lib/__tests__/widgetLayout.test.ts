...
describe('widgetLayout', () => {
  ...
  it('round-trips a saved order', () => {
    saveWidgetOrder('user-1', ['alcohol', 'tip', 'pacing', 'logs', 'challenge'])
    expect(getWidgetOrder('user-1')).toEqual(['alcohol', 'tip', 'pacing', 'logs', 'challenge'])
  })
  ...
  it('appends widgets missing from a stored order', () => {
    ...
    expect(getWidgetOrder('user-1')).toEqual(['alcohol', 'logs', 'pacing', 'tip', 'challenge'])
  })
  ...
  it('keeps separate users isolated', () => {
    saveWidgetOrder('user-1', ['tip', 'alcohol', 'pacing', 'logs', 'challenge'])
